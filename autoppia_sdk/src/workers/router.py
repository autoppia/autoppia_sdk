...
class WorkerRouter():
    """A router class for handling communication with AI workers via Socket.IO or HTTP.

    This class manages the routing and communication with AI worker instances,
    handling configuration retrieval and message processing through both Socket.IO and HTTP channels.
    """
    ...
    def call_http(self, message: str, stream_callback=None):
        """Sends a message to the worker for processing via HTTP.
        ...
        retry_count = 0
        last_error = None
        http_url = f"http://{self.ip}:{self.port}/message"
        ...
        while retry_count < max_retries:
            try:
                ...
                if stream_callback:
                    ...
                    response = requests.post(http_url, json=data, headers=headers, stream=True)
                    ...
                    response = requests.post(http_url, json=data, headers=headers)
                    response.raise_for_status()
                ...
                last_error = e
                if retry_count >= max_retries:
                    raise Exception(f"Failed to call worker via HTTP: {str(e)}")
                time.sleep(1)  # Wait before retrying
